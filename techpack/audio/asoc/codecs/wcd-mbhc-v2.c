...
#ifdef OPLUS_FEATURE_FSA4480
unsigned int wcd_mbhc_component_read(struct snd_soc_component *component, unsigned int reg)
{
	unsigned int val;
	int ret;
...
static void wcd_mbhc_usbc_analog_plug_detect_handler(struct wcd_mbhc *mbhc, bool detection_type)
{
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		...
		if (mbhc->mbhc_cfg->enable_usbc_analog) {
			if ((wcd_mbhc_component_read(mbhc->component, WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG0) & 0x03) == 2) { //0: ZERO, 1: ADC1, 2: ADC2
				pr_info("%s: Config the AMIC TX MUX0 to ZERO\n", __func__);
				snd_soc_component_update_bits(mbhc->component, WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG0, 3, 0);
			} else if ((wcd_mbhc_component_read(mbhc->component, WCD934X_CDC_TX_INP_MUX_ADC_MUX1_CFG0) & 0x03) == 2) {
				pr_info("%s: Config the AMIC TX MUX1 to ZERO\n", __func__);
				snd_soc_component_update_bits(mbhc->component, WCD934X_CDC_TX_INP_MUX_ADC_MUX1_CFG0, 3, 0);
			}
		}
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		...
		if (mbhc->mbhc_cfg->enable_usbc_analog) {
			if ((wcd_mbhc_component_read(component, WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG0) & 0x03) == 2) { //0: ZERO, 1: ADC1, 2: ADC2
				pr_info("%s: Config the AMIC TX MUX0 to ZERO\n", __func__);
