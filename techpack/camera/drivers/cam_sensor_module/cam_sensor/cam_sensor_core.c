...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if ((s_ctrl->sensordata->id_info.sensor_id_reg_addr != 0) && (s_ctrl->sensordata->id_info.sensor_id != 0)) {
		...
		if (slave_info->sensor_id == 0x30d5) {
			if (s_ctrl->sensor_eeprom_same_cci == 0x00) {
		...
		if (s_ctrl->sensordata->id_info.sensor_id != id) {
			...
			if (slave_info->sensor_id == 0x586 && id == 0x03 && lens == 0xe2) {
	...
	if (slave_info->sensor_id == 0x0471) {
		...
		sensor_get_fuseid(s_ctrl, false);
		CAM_INFO(CAM_SENSOR,
	...
	void *arg)
{
	...
	case CAM_GET_DPC_DATA: {
		if (s_ctrl->sensordata->slave_info.sensor_id != 0x0471) {
			rc = -EFAULT;
			goto release_mutex;
		}
