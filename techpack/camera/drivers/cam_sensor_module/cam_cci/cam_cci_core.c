...
static int32_t cam_cci_write_packet(struct cam_cci_ctrl *cci_ctrl,
	int addr, uint8_t *data, uint16_t count)
{
    int i;
	...
	cci_ctrl->cfg.cci_i2c_write_cfg.reg_setting = write_regarray;
...
{
	int32_t rc = 0, exp_byte = 0;
	struct v4l2_subdev *sd = cam_cci_get_subdev(CCI_DEVICE_1);
	struct cci_device *cci_dev = v4l2_get_subdevdata(sd);
	struct camera_cci_transfer* transfer = (struct camera_cci_transfer*)control;
	...
	case CAMERA_CCI_READ:
		...
		if(rc < 0) {
			int i;
			CAM_DBG(CAM_CCI, "cmd %d,rc=%d", transfer->cmd,rc);
			exp_byte = ((cci_ctrl_interface.cfg.cci_i2c_read_cfg.num_byte / 4) + 1);
			CAM_DBG(CAM_CCI, "read exp byte=%d", exp_byte);
			for(i = 0; i < exp_byte; i++){
				CAM_DBG(CAM_CCI, "read byte=0x%x,index=%d",
	...
	case CAMERA_CCI_WRITE:
		...
		if (rc < 0)
			CAM_ERR(CAM_CCI, "write failed, cmd: %d", transfer->cmd);
		break;
	default:
		rc = -ENOIOCTLCMD;
	}
