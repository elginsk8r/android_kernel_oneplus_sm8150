...
void sde_crtc_fingerprint_commit(struct drm_crtc *crtc,
		struct drm_crtc_state *old_state)
{
	...
	if (old_cstate->fingerprint_pressed != cstate->fingerprint_pressed) {
		...
		if (cstate->fingerprint_defer_sync) {
			...
			ret = wait_event_timeout(*drm_crtc_vblank_waitqueue(crtc),
					...
					msecs_to_jiffies(50));
			if (!ret)
				SDE_ERROR("[fingerprint CRTC:%d:%s] target vblank wait timed out\n",
			...
			if (current_vblank == drm_crtc_vblank_count(crtc)) {
				ret = wait_event_timeout(*drm_crtc_vblank_waitqueue(crtc),
					...
					msecs_to_jiffies(17));
				if (!ret)
					SDE_ERROR("[fingerprint CRTC:%d:%s] current vblank wait timed out\n",
						   crtc->base.id, crtc->name);
			}
		}
...
#ifdef OPLUS_BUG_STABILITY
	sde_crtc_fingerprint_commit(crtc, old_state);
#endif /* OPLUS_BUG_STABILITY */
}
...
static int _sde_crtc_check_onscreenfinger(struct sde_crtc_state *cstate,
		struct plane_state *pstates, int cnt)
{
	int fp_index = -1;
...
#ifdef OPLUS_BUG_STABILITY
	rc = _sde_crtc_check_onscreenfinger(cstate, pstates, cnt);
	if (rc)
		return rc;
#endif /* OPLUS_BUG_STABILITY */
