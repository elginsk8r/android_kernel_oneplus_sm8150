...
int dsi_panel_parse_oplus_config(struct dsi_panel *panel)
{
	...
	panel->oplus_priv.is_aod_ramless = utils->read_bool(utils->data,
			"oplus,aod_ramless");
	pr_info("aod ramless mode: %s", panel->oplus_priv.is_aod_ramless ? "true" : "false");
#endif /* OPLUS_FEATURE_AOD_RAMLESS */

#ifdef OPLUS_BUG_STABILITY
	panel->oplus_priv.is_19781_lcd = utils->read_bool(utils->data,
			"oplus,is_19781_lcd");
	pr_info("is_19781_lcd: %s",
	...
	struct dsi_parser_utils *utils)
{
	...
	if (rc) {
		...
		pr_info("oplus,fod-th-brightness is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-on-vblank is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-off-vblank is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-on-delay is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-off-delay is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-on-vblank_above_th is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-off-vblank_above_th is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-on-delay_above_th is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oplus,fod-off-delay_above_th is %d", val);
	}
