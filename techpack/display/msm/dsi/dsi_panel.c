...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->lcd_vci_gpio)) {
		...
		if (rc) {
			DSI_ERR("request for vci_gpio failed, rc=%d\n", rc);
	...
	if (gpio_is_valid(r_config->err_flag_gpio)) {
		...
		if (rc) {
			DSI_ERR("request for err_flag_gpio failed, rc=%d\n", rc);
...
static int dsi_panel_gpio_release(struct dsi_panel *panel)
{
	...
	if (!strcmp(panel->name, "samsung dsc cmd mode oneplus dsi panel")) {
...
#ifdef OPLUS_BUG_STABILITY
	if (true == panel->reset_timing || (!strcmp(panel->name, "samsung sofef03f_m amoled fhd+ panel with DSC")))
			goto exit;
#endif /* OPLUS_BUG_STABILITY */
...
int dsi_panel_gamma_read_address_setting(struct dsi_panel *panel, u16 read_number)
{
	...
	rc = mipi_dsi_dcs_write(dsi, 0xC1, payload, sizeof(payload));
	...
	u32 bl_lvl)
{
	...
	if (oplus_dimlayer_bl_enable_real && bl_lvl > 1)
...
static int dsi_panel_parse_reset_sequence(struct dsi_panel *panel)
{
	...
	DSI_DEBUG("oplus,reset_timing: %s", panel->reset_timing ? "true" : "false");
#endif /* OPLUS_BUG_STABILITY */
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.lcd_vci_gpio))
		pr_debug("%s:%d qcom,panel-vci-gpio gpio not specified\n", __func__, __LINE__);
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	if (rc) {
		...
		DSI_DEBUG("set default brightness to max level\n");
	} else {
	...
	if (rc) {
		pr_debug("[%s] dc backlight unspecified, defaulting to default level 260\n",
			 panel->name);
	...
	pr_debug("[%s] seed backlight max value is %d\n", panel->name, panel->oplus_priv.seed_bl_max);
#endif /* OPLUS_BUG_STABILITY */
