...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->poc)) {
		...
		if (rc) {
			DSI_ERR("[%s] unable to set dir for poc gpio rc=%d\n",
				panel->name, rc);
	...
	if(!strcmp(panel->name, "samsung s6e3fc2x01 cmd mode dsi panel")
		...
		if (gpio_is_valid(panel->vddd_gpio)) {
			...
			if (rc) {
				DSI_ERR("[%s] unable to set dir for vddd_gpio gpio rc=%d\n",
					panel->name, rc);
...
#ifdef OPLUS_BUG_STABILITY
static int dsi_panel_dcs_set_display_brightness(struct mipi_dsi_device *dsi,
			u32 bl_lvl)
{
	u16 brightness = (u16)bl_lvl;
	u8 ctrl_value;
	...
	case 0xE0:
		if(flag_writ == 0 || flag_writ == 3) {
			...
			if(cmp_display_panel_name("SOFEF03F_M"))
				rc = mipi_dsi_dcs_write(dsi, MIPI_DCS_WRITE_CONTROL_DISPLAY,
						   &ctrl_value, sizeof(ctrl_value));
			flag_writ = 2;
		}
...
int dsi_panel_gamma_read_address_setting(struct dsi_panel *panel, u16 read_number)
{
	...
	payload[0] = 0x0A;
	payload[1] = read_number >> 8;
	payload[2] = read_number & 0xff;
...
#else
		rc = dsi_panel_dcs_set_display_brightness(dsi, bl_lvl);
#endif /* OPLUS_BUG_STABILITY */
...
int dsi_panel_pre_prepare(struct dsi_panel *panel)
{
	...
	if (prj_id == 18821 || prj_id == 19801) {
		if (gpio_is_valid(panel->tp1v8_gpio)) {
			rc = gpio_direction_output(panel->tp1v8_gpio, 1);
			if (rc)
				DSI_ERR("[TP] unable to set dir for tp1v8 gpio rc=%d\n", rc);
		}
	}
